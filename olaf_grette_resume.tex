...
\begin{document}
    ...
    \begin{header}
        ...
        \small\color{gray}\textit{Generated \today. Latest version available at \hrefWithoutArrow{https://grette.org/resume}{grette.org/resume}}
    \end{header}
    ...
    \section{Work Experience}
        ...
        \begin{twocolentry}{
            ...
            \textbf{Meta – Production Engineer, Cloud Infrastructure}
        \end{twocolentry}

        \vspace{0.05 cm}
        \begin{onecolentry}
            \textit{Own end-to-end operational reliability for Meta's internal infrastructure stack on bare-metal public cloud instances, expanding GPU supply. Coordinate across 10+ internal teams to close gaps in reliability, performance, and operational integration relative to Meta's on-premises infrastructure.}
        \end{onecolentry}

        \vspace{0.05 cm}
        \begin{onecolentry}
            \begin{highlights}
                \item Define operational priorities, incident response, and acceptance criteria for multi-billion-dollar EC2 capacity; coordinate integration between AWS and Meta maintenance and health systems
                \item Led AWS–Meta escalations for NVLink failures threatening GPU capacity for major launches, including coordination of a broad NVIDIA vBIOS rollout across both companies' systems
                \item Developed interim repair workflows for Oracle GPU hosts affected by backend network instability, preventing production incidents and restoring hosts in under two days versus an expected five through the existing automated repair path
                \item Convened an AWS–Meta operations summit and secured agreement to share operational data and develop shared runbooks for closer incident and maintenance coordination
            \end{highlights}
        \end{onecolentry}
        ...
        \begin{twocolentry}{
            ...
            \textbf{Meta – Production Engineer, Blob Storage Capacity and Efficiency}
        \end{twocolentry}

        \vspace{0.05 cm}
        \begin{onecolentry}
            \textit{Worked as an individual contributor on capacity and storage efficiency for Meta's exabyte-scale central blob storage system serving all business units, coordinating across 7+ engineering teams and driving \textasciitilde\$300M/yr in opex decisions spanning attribution, capacity planning, and rate card design.}
        \end{onecolentry}

        \vspace{0.05 cm}
        \begin{onecolentry}
            \begin{highlights}
                \item Closed a \$45M/yr unattributed-opex leak by reworking resource creation flows to require an accountable budgetary owner, then built a heuristic SQL dataset joining company-wide sources (on-call, HR) that became the basis for reattributing the full \$45M to the correct business units
                \item Secured a \$250M/yr temporary opex loan from the central capacity organization to fund a migration reclaiming 12 EB of HDD, driving the dependency and capacity feasibility analysis and the rate cards and resource translation tables that justified it
                \item Designed the initial rate card for an AI-training blob storage offering (\$25M/yr opex), unblocking its scale-up by modeling the resulting shifts in dependency usage and hardware ordering
            \end{highlights}
        \end{onecolentry}
